...
public class Weapon : MonoBehaviour
{
	...
	void CheckForUserInput()
	{
		...
		if (type == WeaponType.Raycast)
		{
			if (fireTimer >= actualROF && canFire)
			{
				...
				{
					if (gameObject.transform.name == "Katana")
					{
	...
	void OnGUI()
	{
		...
		if (showCurrentAmmo)
		{
			...
			if (type == WeaponType.Raycast || type == WeaponType.Projectile)
				GUI.Label(new Rect(25, Screen.height - 60, 200, 40), "Ammo: " + currentAmmo);
			else if (type == WeaponType.Beam)
				GUI.Label(new Rect(25, Screen.height - 60, 200, 40), "Heat: " + (int)(beamHeat * 100) + "/" + (int)(maxBeamHeat * 100));
		}
	}
	...
	void Fire()
	{
		...
		for (int i = 0; i < shotPerRound; i++)
		{
			...
			{
				float damage = power + player.transform.GetComponent<PlayerStats>().baseDamage;
	...
	void Beam()
	{
		...
		do
		{
			...
			if (Physics.Raycast(ray, out hit, range))
			{
				...
				// Damage
				hit.collider.gameObject.SendMessageUpwards("ChangeHealth", -(beamPower + (player.transform.GetComponent<PlayerStats>().baseDamage * 0.25)), SendMessageOptions.DontRequireReceiver);
